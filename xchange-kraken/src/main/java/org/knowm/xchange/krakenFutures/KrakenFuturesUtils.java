...
public class KrakenFuturesUtils {
  ...
  public static String getProductId(CurrencyPair currencyPair, Object[] args) {
    ...
    KrakenFuturesProduct product =
        getIndexedValue("product", 0, KrakenFuturesProduct.class, null, true, args);
  ...
  public static <T> T getIndexedValue(
      String name,
      int index,
    ...
    if (args.length >= index + 1 && expectedClass != null) {
      if (expectedClass.isAssignableFrom(args[index].getClass())) {
        ...
        throw new ExchangeException(
            String.format(
                "Invalid type of parameter #%d (%s): expected %s but actual %s",
                index, name, expectedClass.getName(), args[index].getClass()));
      }
    }
    if (defaultValue != null) {
      log.warn(
          "Parameter '{}' was not correctly specified, so the default value {} is used",
          name,
          defaultValue);
      return defaultValue;
    }
    if (failIfMissing) {
      throw new ExchangeException(
          String.format("Parameter #%d (%s) is not specified", index, name));
    }
    return null;
  }
}
