...
public class KrakenFuturesAdapters {
    ...
    public static Ticker adaptTicker(KrakenFuturesTicker krakenTicker) {
        ImmutableTriple<KrakenFuturesProduct, CurrencyPair, LocalDate> productItems = KrakenFuturesProduct.parseProductId(krakenTicker.getSymbol());
    ...
    public static OrderBook adaptOrderbook(KrakenFuturesOrderBookResult krakenOrderbook, CurrencyPair currencyPair) {
        Stream<LimitOrder> askStream = krakenOrderbook.getOrderBook().getAsks().stream()
                .map(ask -> new LimitOrder(Order.OrderType.ASK, ask[1], currencyPair, null, null, ask[0]));
